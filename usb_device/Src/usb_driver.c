...
static void RXflvl_handler()
{
   ...
   case 0x03:                                                                               // out transfer completed
      ...
      SET_bit(OUT_ENDPOINT(ep_num)->DOEPCTL, USB_OTG_DOEPCTL_CNAK | USB_OTG_DOEPCTL_EPENA); // re-enable the endpoint
 ...
 *
 */
static void write_packet(endp_no num, uint8_t const *buffer, uint16_t size)
{
...
usb_status reset_fifo()
{
   ...
   for(uint8_t i =1; i< ENDPOINTCOUNT; i++)
   {
    ...
    if( USB_OTG_GLOBAL->GRXFSIZ  || USB_OTG_GLOBAL->DIEPTXF0_HNPTXFSIZ ||\
       USB_OTG_GLOBAL->DIEPTXF[1] || USB_OTG_GLOBAL->DIEPTXF[2] || USB_OTG_GLOBAL->DIEPTXF[3] )
       {
          return error;
...
static void OUT_endpoint_Irqhandler()
{
   ...
   if (READ_bit(status, USB_OTG_DOEPINT_XFRC))
   {
      ...
      // also reenable the endpoint and clear the nak bit
      SET_bit(OUT_ENDPOINT(num)->DOEPCTL, USB_OTG_DOEPCTL_EPENA | USB_OTG_DOEPCTL_CNAK);
   }
 ...
 */
static usb_status initialize_core()
{
...
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
usb_status ActivateEP(endp_no num, eptype type )
{
   ...
   else 
   {
      if(type == IN)
      {
        // check if endpoint is configured or not
...
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
usb_status DeactivateEP(endp_no num, eptype type)
{
...
usb_status setstall(endp_no num, eptype type )
{
...
usb_status clearstall(endp_no num, eptype type)
{
   // first make sure that endpoint is enable 
      if( type == IN)
      {
...
void configure_endpoint0(endp_size size)
{
   ...
   SET_bit(IN_ENDPOINT(0)->DIEPCTL,  USB_OTG_DIEPCTL_CNAK | VAL2FLD(USB_OTG_DIEPCTL_MPSIZ, size));

   // clear the out endpoint 0 and enable for the data transmissoin
   SET_bit(OUT_ENDPOINT(0)->DOEPCTL, USB_OTG_DOEPCTL_CNAK | USB_OTG_DOEPCTL_EPENA | VAL2FLD(USB_OTG_DOEPCTL_MPSIZ, size));
...
void deconfigure_in_endpoint(endp_no num)
{
   ...
   WRITE_reg(IN_ENDPOINT(num)->DIEPCTL ,0);
   // flush the tx fifo
...
void deconfigure_out_endpoint(endp_no num)
{
 ...
 // flush the rx fifo
   ...
   WRITE_reg( OUT_ENDPOINT(num)->DOEPCTL , 0x00008000U );
   if (num == 0)
      return;
   // disable the endpoint
   if (READ_bit(OUT_ENDPOINT(num)->DOEPCTL, USB_OTG_DOEPCTL_EPENA))
